const appModel = (function() {
  ...
  class AppModel {
    ...
    getWeatherHourly(longitude, latitude) {
      return fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=bb5707b9a02b5c08635c421eed9e2690`
      )
        .then(response => response.json())
    ...
    getCurrentWeather(longitude, latitude) {
      return fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=bb5707b9a02b5c08635c421eed9e2690`
      )
        .then(response => response.json())
        .catch(error => console.error(error));
    }
    async fetchCurrentWeather() {
      currentTemperatureData = await this.getCurrentWeather(
        longitude,
        latitude
      );
      currentTemperature = (
        currentTemperatureData.main.temp - KELVIN_CONSTANT
      ).toFixed();
      weatherDescription = currentTemperatureData.weather[0].main;
      currentWeather.forEach(resolveFn =>
        resolveFn(currentTemperature, weatherDescription)
      );
    }
    ...
    async getWeatherDescription() {
      if (weatherDescription) {
        ...
        const description = new Promise(resolve => {
          currentweatherDescription.push(weatherDescription =>
